...
///
/// Only includes user messages and tool results — not assistant responses.
/// Assistant messages are generated by the LLM itself and can't contain
/// injection. This keeps the screening payload small enough for the SLM
/// context window, even in long conversations.
///
/// Screening: system prompt + all user messages + tool_result blocks.
/// Skipped: assistant messages (safe, self-generated).
pub fn screen_payload_to_string(payload: &AnthropicScreenPayload) -> String {
    ...
    for msg in &payload.messages {
        match msg.role.as_str() {
            ...
            // Skip assistant messages — they're self-generated, not attack surface
            _ => {}
        }
    }
    parts.join("\n")
