...
<div class="panel" id="panel-evidence"><div class="card"><h2>Evidence Chain</h2><p id="evidence-info">Loading...</p><div id="evidence-table"></div></div></div>
<div class="panel" id="panel-vault"><div class="card"><h2>Credential Vault Scan</h2><div id="vault-summary"></div><div id="vault-table"></div></div></div>
<div class="panel" id="panel-access"><div class="card"><h2>Service Access Log</h2><div id="access-table"></div></div></div>
<div class="panel" id="panel-memory"><div class="card"><h2>Memory Integrity</h2><p id="memory-info">Loading...</p><div id="memory-table"></div></div></div>
...
function typeBadge(t){
  const colors={WriteBarrier:'red',MemoryIntegrity:'yellow',ApiCall:'blue',ModeChange:'green',VaultDetection:'red',SlmAnalysis:'yellow'};
...
function renderEvidence(e){
  const el=document.getElementById('evidence-table');
  if(!e.recent_receipts||e.recent_receipts.length===0){el.innerHTML='<p class="empty-state">No receipts recorded yet.</p>';return;}
  let h='<table class="dtable"><tr><th>Seq</th><th>Time</th><th>Type</th><th>Action</th><th>Outcome</th></tr>';
  for(const r of e.recent_receipts){
    h+='<tr><td>'+r.seq+'</td><td>'+fmtTime(r.ts_ms)+'</td><td>'+typeBadge(r.receipt_type)+'</td><td>'+(r.action||'—')+'</td><td>'+(r.outcome||'—')+'</td></tr>';
  }
  h+='</table>';
  el.innerHTML=h;
}
function renderVault(v){
  ...
  if(v.total_secrets===0){sum.innerHTML='<p class="empty-state">No credentials detected in recent traffic.</p>';tbl.innerHTML='';return;}
  let s='<p style="color:#f85149;font-size:16px;margin-bottom:8px">'+v.total_secrets+' credential(s) detected</p>';
  const types=Object.entries(v.by_type);
  if(types.length>0){s+='<p style="color:#8b949e;font-size:13px;margin-bottom:8px">By type: '+types.map(([k,n])=>k+' ('+n+')').join(', ')+'</p>';}
  sum.innerHTML=s;
  if(!v.recent_findings||v.recent_findings.length===0){tbl.innerHTML='';return;}
  let h='<table class="dtable"><tr><th>Time</th><th>Type</th><th>Preview</th></tr>';
  for(const f of v.recent_findings){
    h+='<tr><td>'+fmtTime(f.detected_at_ms)+'</td><td><span class="badge badge-red">'+f.credential_type+'</span></td><td>'+f.masked_preview+'</td></tr>';
  }
  h+='</table>';
  tbl.innerHTML=h;
}
function renderAccess(a){
  ...
  if(!a.entries||a.entries.length===0){el.innerHTML='<p class="empty-state">No API calls recorded yet. Route traffic through Aegis to see entries.</p>';return;}
  let h='<table class="dtable"><tr><th>Seq</th><th>Time</th><th>Method</th><th>Path</th><th>Status</th><th>Duration</th></tr>';
  for(const e of a.entries){
    const sc=e.status?(e.status<400?'badge-green':'badge-red'):'badge-gray';
    h+='<tr><td>'+e.seq+'</td><td>'+fmtTime(e.ts_ms)+'</td><td>'+e.method+'</td><td>'+e.path+'</td><td><span class="badge '+sc+'">'+(e.status||'—')+'</span></td><td>'+(e.duration_ms?e.duration_ms+'ms':'—')+'</td></tr>';
  }
  h+='</table>';
  el.innerHTML=h;
}
function renderMemory(m){
  ...
  if(m.tracked_files===0&&m.changes_detected===0){el.innerHTML='<p class="empty-state">No memory files detected yet.</p>';return;}
  let h='<div style="margin-top:12px">';
  h+='<div class="grid" style="margin-bottom:12px">';
  h+='<div class="card"><div class="stat">'+m.tracked_files+'</div><div class="stat-label">Tracked Files</div></div>';
  h+='<div class="card"><div class="stat'+(m.changes_detected>0?' status-warn':'')+'">'+m.changes_detected+'</div><div class="stat-label">Changes Detected</div></div>';
  h+='<div class="card"><div class="stat'+(m.unacknowledged_changes>0?' status-error':'')+'">'+m.unacknowledged_changes+'</div><div class="stat-label">Unacknowledged</div></div>';
  h+='<div class="card"><div class="stat" style="font-size:16px">'+(m.last_scan_ms?fmtTime(m.last_scan_ms):'Never')+'</div><div class="stat-label">Last Scan</div></div>';
  h+='</div></div>';
  el.innerHTML=h;
}
