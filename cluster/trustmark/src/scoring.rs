...
impl TrustmarkScore {
    ...
    pub fn compute(signals: &LocalSignals) -> Self {
        ...
        Self { total, dimensions: dims, computed_at_ms }
    }
    ...
    fn score_persona_integrity(s: &LocalSignals) -> DimensionScore {
        let (value, reason) = if s.protected_files_total == 0 {
            ...
            ((1.0 - penalty).max(0.0),
             format!("{} between-session tamper(s) detected", s.between_session_tampers))
        } else {
            ...
            (value, format!(
                ...
                match s.manifest_signature_valid {
                    Some(true) => "valid", Some(false) => "INVALID", None => "absent",
                }
            ))
        };
    ...
    fn score_chain_integrity(s: &LocalSignals) -> DimensionScore {
        let (value, reason) = match s.chain_verified {
            Some(true) => (1.0, format!("chain intact ({} receipts)", s.chain_receipt_count)),
            Some(false) => (0.0, "chain verification FAILED".into()),
            None => {
                if s.chain_receipt_count > 0 {
                    (0.7, format!("{} receipts, not yet verified", s.chain_receipt_count))
                } else {
    ...
    fn score_temporal_consistency(s: &LocalSignals) -> DimensionScore {
        let (value, reason) = if s.receipt_timestamps.len() < 3 {
            (0.5, format!("only {} timestamps, need 3+", s.receipt_timestamps.len()))
        } else {
            let intervals: Vec<f64> = s.receipt_timestamps.windows(2)
            ...
            if mean == 0.0 {
                ...
                (value, format!("{} intervals, CV={:.2} ({})", intervals.len(), cv,
                    if cv < 0.5 { "very consistent" }
                    else if cv < 1.0 { "consistent" }
                    else if cv < 2.0 { "somewhat bursty" }
                    else { "very bursty" }
                ))
            }
        };
...
mod tests {
    ...
    fn fresh_install_moderate_score() {
        ...
        assert!(score.total > 0.3 && score.total < 0.7,
            "fresh install should be moderate: {}", score.total);
    ...
    fn perfect_signals_high_score() {
        let signals = LocalSignals {
            protected_files_total: 9, protected_files_intact: 9,
            manifest_signature_valid: Some(true), between_session_tampers: 0,
            chain_verified: Some(true), chain_receipt_count: 10000,
            vault_scans_total: 500, vault_leaks_detected: 0, vault_leaks_redacted: 0,
            receipt_timestamps: (0..288).map(|i| i * 300_000).collect(),
            receipts_last_24h: 288, volume_baseline: Some(100),
            relay_forwarded: 100, relay_failed: 0,
        };
        let score = TrustmarkScore::compute(&signals);
        assert!(score.total > 0.90, "perfect should be > 0.90: {}", score.total);
    }

    #[test]
    fn terrible_signals_low_score() {
        let signals = LocalSignals {
            protected_files_total: 9, protected_files_intact: 3,
            manifest_signature_valid: Some(false), between_session_tampers: 5,
            chain_verified: Some(false), chain_receipt_count: 10,
            vault_scans_total: 100, vault_leaks_detected: 50, vault_leaks_redacted: 10,
            receipt_timestamps: vec![1000, 2000, 100_000_000],
            receipts_last_24h: 3, volume_baseline: Some(100),
            relay_forwarded: 10, relay_failed: 90,
        };
        let score = TrustmarkScore::compute(&signals);
        assert!(score.total < 0.25, "terrible should be < 0.25: {}", score.total);
    }
    ...
    fn persona_all_intact_is_1() {
        let s = LocalSignals {
            protected_files_total: 9, protected_files_intact: 9,
            manifest_signature_valid: Some(true), ..Default::default()
        };
    ...
    fn persona_between_session_tamper_drops() {
        let s = LocalSignals {
            protected_files_total: 9, protected_files_intact: 9,
            manifest_signature_valid: Some(true), between_session_tampers: 2,
            ..Default::default()
        };
        let score = TrustmarkScore::compute(&s);
    ...
    fn chain_verified_is_1() {
        let s = LocalSignals { chain_verified: Some(true), chain_receipt_count: 5000, ..Default::default() };
        let score = TrustmarkScore::compute(&s);
    ...
    fn chain_broken_is_0() {
        let s = LocalSignals { chain_verified: Some(false), chain_receipt_count: 5000, ..Default::default() };
        let score = TrustmarkScore::compute(&s);
    ...
    fn vault_no_leaks_is_1() {
        let s = LocalSignals { vault_scans_total: 1000, vault_leaks_detected: 0, ..Default::default() };
        let score = TrustmarkScore::compute(&s);
    ...
    fn vault_leaks_with_redaction() {
        let s = LocalSignals {
            vault_scans_total: 100, vault_leaks_detected: 10, vault_leaks_redacted: 8,
            ..Default::default()
        };
        let score = TrustmarkScore::compute(&s);
    ...
    fn temporal_regular_is_high() {
        ...
        let score = TrustmarkScore::compute(&s);
        assert!(score.dimensions[3].value > 0.9, "{}", score.dimensions[3].value);
    }
    ...
    fn temporal_too_few_is_default() {
        let s = LocalSignals { receipt_timestamps: vec![1000, 2000], ..Default::default() };
        let score = TrustmarkScore::compute(&s);
    ...
    fn volume_above_baseline_caps_at_1() {
        let s = LocalSignals { receipts_last_24h: 200, volume_baseline: Some(100), ..Default::default() };
        let score = TrustmarkScore::compute(&s);
    ...
    fn volume_half_baseline() {
        let s = LocalSignals { receipts_last_24h: 50, volume_baseline: Some(100), ..Default::default() };
        let score = TrustmarkScore::compute(&s);
    ...
    fn weights_sum_to_one() {
        let sum = WEIGHT_PERSONA_INTEGRITY + WEIGHT_CHAIN_INTEGRITY + WEIGHT_VAULT_HYGIENE
            + WEIGHT_TEMPORAL_CONSISTENCY + WEIGHT_RELAY_RELIABILITY + WEIGHT_CONTRIBUTION_VOLUME;
